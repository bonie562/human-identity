...
function Ravewave({ workData }) {
  const raveContents = Array.isArray(workData?.contents)
    ? workData.contents
    : [];
  ...
  while (i < raveContents.length) {
    if (
      twinRows.includes(groupedImages.length) &&
      i + 1 < raveContents.length
    ) {
      groupedImages.push([raveContents[i], raveContents[i + 1]]);
      i += 2;
    } else {
      groupedImages.push([raveContents[i]]);
      i += 1;
    }
  }
  ...
  return (
    <div className="mb-20">
      ...
      <ProjectInfo projectInfoText={infoText.raveInfo} />
      
    </div>
  );
}

export default Ravewave;
